...
export function drawThreeGeo(json, radius, shape, materialOptions, container) {
  ...
  function drawLine(x_values, y_values, z_values, options) {
    ...
    let hue = 0.2 + Math.random() * .2
    if(Math.random() > 0.5) {
      hue -= .3
    }
    const color = options.color ? options.color : new THREE.Color().setHSL(hue, 1.0, .5)

    const line_material = new THREE.LineBasicMaterial({ ...options, color });
